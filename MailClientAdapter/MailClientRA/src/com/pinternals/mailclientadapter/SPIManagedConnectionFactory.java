...
public class SPIManagedConnectionFactory
  implements ManagedConnectionFactory, Serializable, Runnable, ManagedConnectionFactoryActivation
{
    ...
    throws ResourceException
  {
    ...
    try
    {
      synchronized (synchronizer)
      {
        ...
        TRACE.infoT("SpiManagedConnectionFactory()", XIAdapterCategories.CONNECT_AF, "This SPIManagedConnectionFactory has the GUID: " + mcfLocalGuid.toString());
      }
    }
    catch (Exception e)
    {
...
//  }
  ...
  public boolean equals(Object obj)
  {
    ...
    if ((obj instanceof SPIManagedConnectionFactory))
    {
      ...
      if ((adapterNamespace.equals(other.getAdapterNamespace())) && (adapterType.equals(other.getAdapterType())) && (addressMode.equals(other.getAddressMode()))) {
        equal = true;
      }
    }
  ...
  public String getAddressMode()
  {
    ...
    TRACE.debugT("getAddressMode()", XIAdapterCategories.CONNECT, "Address determination mode =" + addressMode);
  ...
  public void startTimer()
  {
    ...
    if (mcfLocalGuid != null) {
      try
      {
        controlTimer.scheduleAtFixedRate(new XIManagedConnectionFactoryController(this, ctx), 120000L, 60000L);
      }
      catch (Exception e)
      {
  ...
  public void run()
  {
    ...
    try
    {
      ...
      while ((notSet) && (numTry < propWaitNum)) {
          ...
          numTry++;
          TRACE.debugT("run()", XIAdapterCategories.CONNECT_AF, "MCF waits for setter completion. Try: {0} of {1}.", new Object[] { Integer.toString(numTry), Integer.toString(propWaitNum) });
          try
          {
      ...
      {
        TRACE.catching("run()", e);
        TRACE.errorT("run()", XIAdapterCategories.CONNECT_AF, "Cannot instatiate the MailClientAdapter module processor bean. The inbound processing is stopped. Exception:" + e.toString());
        threadStatus = 2;
      }
...
//              sendMessageFromFile("/dev/null", channel, processMode, qos, raiseError, channelAddressMode, set_asma);
              // test code
              TRACE.warningT("XIManagedConnectionFactoryController.run()", XIAdapterCategories.CONNECT_AF, "Code for polling Mail Sender @@@");
              
              InitialContext ctx = new InitialContext();
              Session ses = (Session)ctx.lookup("java:comp/env/mail/MailSession");
              TRACE.debugT(SIGNATURE, "Session:" + ses + " " + ses.getStore().toString());
              Store st = ses.getStore("imaps");
              st.connect();
              Folder f = st.getDefaultFolder();
              int mc;
              TRACE.debugT(SIGNATURE, "Store: {0}, Folder: {1},{2}", new Object[]{st, f, f.getFullName()});
              
              f = st.getFolder("INBOX");
              mc = f.getMessageCount();
              TRACE.debugT(SIGNATURE, "Store: {0}, Folder: {1}, messages count: {2}", new Object[]{st, f, mc});
              ...
              Binding b = CPAFactory.getInstance().getLookupManager().getBindingByChannelId(channel.getObjectId());
              Message msg = mf.createMessageRecord(b.getFromParty(), b.getFromService(), b.getToParty(), b.getToService(), b.getActionName(), b.getActionNamespace());
              ...
              p.setDescription("XI AF Sample Adapter Input: XML document as MainDocument");              
              msg.setDocument(p);
  ...
  private void sendMessageFromFile1(String inFileName, Channel channel, String processMode, String qos, String raiseError, String channelAddressMode, boolean set_asma)
  {
...
//      if (!inputFile.exists()) {
//   	    TRACE.warningT("sendMessageFromFile(String inFileName)", "No filename '"+inFileName+"' exist");
//        fileRead = false;
//      }
//    }
//    catch (Exception e)
...
//    TRACE.infoT("sendMessageFromFile(String inFileName)", "External message ID is '" + extMsgId + "', fileRead="+fileRead+",processMode="+processMode+",qos="+qos+",inFileName="+inFileName);
//    if ((fileRead == true) && (0 != processMode.compareToIgnoreCase("test")) && (
...
//            
//            com.sap.aii.af.service.cpa.Service fromXIService = normalizer.getXIService(fromParty, "GLN", fromService);
...
//            }
//            com.sap.aii.af.service.cpa.Party fromXIParty = normalizer.getXIParty("009", "GLN", fromParty);
...
//            }
//            com.sap.aii.af.service.cpa.Service toXIService = normalizer.getXIService(toParty, "GLN", toService);
...
//            }
//            com.sap.aii.af.service.cpa.Party toXIParty = normalizer.getXIParty("009", "GLN", toParty);
        ...
        if (!qos.equalsIgnoreCase("BE"))
        {
          ...
          try
          {
            ...
            if (0 != processMode.compareToIgnoreCase("test")) {
              messageIDMapper.createIDMap(extMsgId, xiMsgId, System.currentTimeMillis() + 86400000L, true);
            }
            if (0 == raiseError.compareToIgnoreCase("rollback"))
            {
              audit.addAuditLogEntry(amk, AuditLogStatus.ERROR, "Channel error mode is set to rollback. An Exception is thrown now to demonstrate a rollback behavior.");
              audit.flushAuditLogEntries(amk);
              ...
              try
              {
                ...
                mm.reportProcessStatus(adapterNamespace, adapterType, ChannelDirection.SENDER, ProcessState.FATAL, "Rollback triggered (as demo) since channel error mode was set to rollback", pc);
              }
              catch (Exception e)
              {
                TRACE.catching("sendMessageFromFile(String inFileName)", e);
...
//            }
  ...
  {
    int keymaxlen = 127;
    String extMsgId = "JCASample::" + (f == null ? "NULL" : new StringBuilder().append(f.getAbsolutePath()).append("_").append(f.lastModified()).toString());
