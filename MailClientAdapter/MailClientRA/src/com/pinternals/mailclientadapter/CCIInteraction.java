...
public class CCIInteraction implements XIInteraction {
	...
	public CCIInteraction(Connection cciConnection) throws ResourceException {
		...
		if (cciConnection == null) {
			ResourceException re = new ResourceException(
					"No related CCI connection in Interaction (cciConnection is null).");
		...
		if (this.mc == null) {
			ResourceException re = new ResourceException(
					"No related managed connection in CCI connection (mc is null).");
			TRACE.throwing(SIGNATURE, re);
			throw re;
		}
		...
		if (this.mcf == null) {
			ResourceException re = new ResourceException(
					"No related managed connection factory in managed connection (mcf is null).");
			TRACE.throwing(SIGNATURE, re);
			throw re;
		}
	...
	public boolean execute(InteractionSpec ispec, Record input, Record output)
		...
		if (!(output instanceof XIMessageRecord)) {
			ResourceException re = new ResourceException(
					"Output record is no XI AF XIMessageRecord.");
			TRACE.throwing(SIGNATURE, re);
			throw re;
		}
		...
		try {
			...
			TRACE.errorT(SIGNATURE, XIAdapterCategories.CONNECT, "SOA.apt_sample.0002",
					"Exception during output record transfer. Reason: {0}", new Object[] { e
							.getMessage() });
			ResourceException re = new ResourceException("Output record cannot be filled. Reason: "
					+ e.getMessage());
			TRACE.throwing(SIGNATURE, re);
			throw re;
		}
	...
	public Record execute(InteractionSpec ispec, Record input) throws ResourceException {
		...
		if (method.compareTo("Send") == 0) {
			...
			ResourceException re = new ResourceException("Unknown function name in ispec: "
					+ method);
			TRACE.throwing(SIGNATURE, re);
			throw re;
		}
	...
	private Record send(InteractionSpec ispec, Record input, SPIManagedConnection mc)
			throws ResourceException {
		...
		if (payText.indexOf("<DeliveryException>") != -1) {
			TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT,
					"Payload contains the <DeliverableException> tag "
							+ "that causes a XIDeliveryException for testing purposes!");
			XIAdapterException de = new XIAdapterException(
					"XI AF JCA sample ra cannot deliver the message (test)");
			this.audit.addAuditLogEntry(amk, AuditLogStatus.ERROR,
					"Payload contains the <DeliverableException> tag that causes "
		...
		if (payText.indexOf("<RecoverableException>") != -1) {
			TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT,
					"Payload contains the <RecoverableException> tag that causes a"
							+ " XIRecoverableException for testing purposes!");
			XIAdapterException re = new XIAdapterException(
					"XI AF JCA sample ra cannot temporarily deliver the message (test)");
			this.audit.addAuditLogEntry(amk, AuditLogStatus.ERROR,
					"Payload contains the <RecoverableException> tag that causes a"
							+ " XIRecoverableException for testing purposes!");
			this.audit.flushAuditLogEntries(amk);
		...
		if (payText.indexOf("<FatalTraceOn>") != -1) {
			TRACE.fatalT(SIGNATURE, XIAdapterCategories.CONNECT_AF, "SOA.apt_sample.0050",
					"Ignore this and the following trace messages in the method send()."
							+ " It is just a sample for using the trace API!");

			TRACE.fatalT(SIGNATURE, XIAdapterCategories.CONNECT_AF, "SOA.apt_sample.0051",
					"A fatal trace message with signature, category and text");

			TRACE.fatalT(SIGNATURE, XIAdapterCategories.CONNECT_AF, "SOA.apt_sample.0052",
					"A fatal trace message with signature, category, text and {0}.",
					new Object[] { "parameters" });

			TRACE.fatalT(SIGNATURE, "SOA.apt_sample.0053",
					"A fatal trace message with signature and text only");

			TRACE.fatalT(SIGNATURE, "SOA.apt_sample.0054",
					"A fatal trace message with signature, text and {0}",
					new Object[] { "parameters" });
		}
		...
		try {
			...
			this.audit.addAuditLogEntry(amk, AuditLogStatus.ERROR,
					"Unable to write message into file.");
			this.audit.flushAuditLogEntries(amk);
			TRACE.throwing(SIGNATURE, de);
			throw de;
		}
		this.audit.addAuditLogEntry(amk, AuditLogStatus.SUCCESS,
				"Async. message was forwarded succesfully to the file system");
		this.audit.flushAuditLogEntries(amk);

		MessageKey msgKey = msg.getMessageKey();
		TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT,
				"Message key {0} with this data received: ID: {1} Direction: {2}",
				new Object[] { msgKey.toString(), msgKey.getMessageId(),
						msgKey.getDirection().toString() });
		if (payText.indexOf("<AppAckOn>") == -1) {
			try {
				...
				TRACE.catching(SIGNATURE, e);
				TRACE.warningT(SIGNATURE, XIAdapterCategories.CONNECT,
						"Not supported Acks cannot be published!");
			}
		} else {
	...
	private String[] getMappedHeaderFieldsAndNormalize(String channelID, Message msg) {
		...
		try {
			TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT,
					"Get receiver agreement with OutboundRuntimeLookup now.");
			CPAOutboundRuntimeLookupManager outLookup = CPAFactory.getInstance()
					.createOutboundRuntimeLookupManager(this.mcf.getAdapterType(),
							this.mcf.getAdapterNamespace(), msg.getFromParty().toString(),
							msg.getToParty().toString(), msg.getFromService().toString(),
							msg.getToService().toString(), msg.getAction().getName(),
							msg.getAction().getType());

			Binding binding = outLookup.getBinding();

			TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT,
					"Get receiver agreement for channel ID {0} now.", new Object[] { channelID });
			Binding bindingByChannel = CPAFactory.getInstance().getLookupManager()
					.getBindingByChannelId(channelID);
			...
			TRACE
					.debugT(
							SIGNATURE,
							XIAdapterCategories.CONNECT,
							"Get header mappings for message with ID {0} and receiver agreement with ID {1} now.",
							new Object[] { msg.getMessageId(), binding.getObjectId() });
			try {
			...
			TRACE.catching(SIGNATURE, e);
			TRACE.errorT(SIGNATURE, XIAdapterCategories.CONNECT, "SOA.apt_sample.0004",
					"Exception during header mapping. Reason: {0}. Error will be ignored.",
		...
		try {
			...
			fromServiceIdentifier = normalizer.getAlternativeServiceIdentifier(fromParty,
					fromService, "GLN");
			if ((fromServiceIdentifier != null)
					&& (fromServiceIdentifier.getServiceIdentifier() != null)
					&& (fromServiceIdentifier.getServiceIdentifier().length() > 0)) {
				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT,
						"Address normalization for service: {0} is: {1}", new Object[] {
								fromService, fromServiceIdentifier.getServiceIdentifier() });
				fromService = fromServiceIdentifier.getServiceIdentifier();
			} else {
				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT,
						"Address normalization is not defined for service: {0}",
						new Object[] { fromService });
			}
			...
			if ((fromPartyIdentifier != null) && (fromPartyIdentifier.getParty() != null)
				...
				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT,
						"Address normalization for party: {0} is: {1}", new Object[] { fromParty,
								fromPartyIdentifier.getPartyIdentifier() });
				fromParty = fromPartyIdentifier.getPartyIdentifier();
			} else {
				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT,
						"Address normalization is not defined for party: {0}",
						new Object[] { fromParty });
			}
			toServiceIdentifier = normalizer.getAlternativeServiceIdentifier(toParty, toService,
					"GLN");
			if ((toServiceIdentifier != null)
					&& (toServiceIdentifier.getServiceIdentifier() != null)
					&& (toServiceIdentifier.getServiceIdentifier().length() > 0)) {
				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT,
						"Address normalization for service: {0} is: {1}", new Object[] { toService,
								toServiceIdentifier.getServiceIdentifier() });
				toService = toServiceIdentifier.getServiceIdentifier();
			} else {
				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT,
						"Address normalization is not defined for service: {0}",
						new Object[] { toService });
			}
			...
			if ((toPartyIdentifier != null) && (toPartyIdentifier.getParty() != null)
				...
				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT,
						"Address normalization for party: {0} is: {1}", new Object[] { toParty,
								toPartyIdentifier.getPartyIdentifier() });
			...
			TRACE.catching(SIGNATURE, e);
			TRACE.errorT(SIGNATURE, XIAdapterCategories.CONNECT, "SOA.apt_sample.0005",
					"Exception during address normalization. Reason: {0}. Error will be ignored.",
					new Object[] { e.getMessage() });
		}
	...
	private String[] getFaultIF(String channelID) {
		...
		try {
			TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT,
					"Get channel CPA object with channelID {0}", new Object[] { channelID });
			Channel channel = (Channel) CPAFactory.getInstance().getLookupManager().getCPAObject(
			...
			TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT_AF,
					"Read this fault interface value: Name: {0} Namespace: {1}", new Object[] {
							result[0], result[1] });
		} catch (Exception e) {
			TRACE.catching(SIGNATURE, e);
			...
			TRACE
					.debugT(
							...
							"Fault interface cannot be read from channel configuration due to {0}. Take defaults value: Name: {1} Namespace: {2}",
							new Object[] { e.getMessage(), result[0], result[1] });
		}
		TRACE.exiting(SIGNATURE);
		return result;
	}
	...
	private Record call(InteractionSpec ispec, Record input, SPIManagedConnection mc)
			throws ResourceException {
		...
		if (file == null) {
			ResourceException re = new ResourceException(
					"No related file stream resource in managed connection (file is null).");
			TRACE.throwing(SIGNATURE, re);
			throw re;
		}
		...
		try {
			...
			if (payText.indexOf("<ApplicationError>") == -1) {
				output = new XIMessageRecordImpl(msg.getToParty(), msg.getFromParty(), msg
						.getToService(), msg.getFromService(), msg.getAction());

				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT_AF,
						"Retrieve XI message from output: " + output.toString());
				Message response = output.getXIMessage();

				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT_AF,
						"Create payload of synchronous response: " + response.toString());
				XMLPayload xp = response.createXMLPayload();

				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT_AF,
						"Fill payload of synchronous response: " + xp.toString());
				...
				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT_AF,
						"Set payload of synchronous response.");
				response.setDocument(xp);
				String requestId = msg.getMessageId();
				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT_AF,
						"Set RefToMsgId of synchronous response to: " + requestId);
				response.setRefToMessageId(requestId);

				this.audit.addAuditLogEntry(amk, AuditLogStatus.SUCCESS,
						"Sync. message was forwarded succesfully to the file system");
			} else {
				TRACE
						.debugT(
								SIGNATURE,
								XIAdapterCategories.CONNECT,
								"Payload contains the <ApplicationError> tag that causes a application error response for testing purposes!");
				this.audit.addAuditLogEntry(amk, AuditLogStatus.ERROR,
						"Simulate application error response now.");

				String[] faultIF = getFaultIF(mc.getChannelID());
				Action action = new Action(faultIF[0], faultIF[1]);
				output = new XIMessageRecordImpl(msg.getToParty(), msg.getFromParty(), msg
						.getToService(), msg.getFromService(), action);
				...
				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT_AF,
						"Create payload of synchronous error response: " + response.toString());
				...
				if (payText.indexOf("ApplicationErrorBinaryPayload") != -1) {
					TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT_AF,
							"Fill binary payload of synchronous ApplicationError response");
					...
					TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT_AF,
							"Fill text payload of synchronous ApplicationError response");
					...
					TRACE
							.debugT(SIGNATURE, XIAdapterCategories.CONNECT_AF,
									"Fill XML payload of synchronous ApplicationError response with binary attachment");
					xp
							.setText("<?xml version=\"1.0\" encoding=\"UTF-8\"?><Failure><Error>Error simulated, ApplicationError contains XML payload with binary attachment</Error></Failure>");
					xp.setContentType("application/xml");
					...
					TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT_AF,
							"Fill XML payload of synchronous ApplicationError response");
					xp
							.setText("<?xml version=\"1.0\" encoding=\"UTF-8\"?><Failure><Error>Error simulated, ApplicationError contains XML payload only</Error></Failure>");
					xp.setContentType("application/xml");
				}
				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT_AF,
						"Set payload of synchronous error response.");
				response.setDocument(xp);

				String requestId = msg.getMessageId();
				TRACE.debugT(SIGNATURE, XIAdapterCategories.CONNECT_AF,
						"Set RefToMsgId of synchronous error response to: " + requestId);
				response.setRefToMessageId(requestId);
				...
				errorInfo
						.setAttribute(
								"AdditionalErrorText",
								"MainDocument has contained the <ApplicationError> element that triggers the JCA adapter to create an app error response as demo!");
